                    ...
                    {suggestedUsers?.length > 0 ? suggestedUsers.map((user) => {
                        ...
                        >
                            <img
                                ...
                                alt={user?.name}
                                loading='lazy'
                                className={`w-12 h-12 max-[900px]:h-14 max-[900px]:w-14 max-[450px]:h-11 max-[450px]:w-11 ${onlineUsers.includes(user._id) ? 'border-green-600' : 'border-red-600'} border-[3px] p-1 rounded-full object-cover`}
                            />
                            <div className="flex-1 min-w-0 max-[900px]:hidden max-[600px]:block">
                                <h3 className="font-semibold max-[450px]:font-medium max-[450px]:text-sm ">{user.username}</h3>
                                <p className="text-xs max-[600px]:text-sm max-[450px]:text-xs text-gray-500 truncate">{unreadChats?.some(e => e.senderId === user._id) ? <span className='font-semibold text-white'>{count} new {count > 1 ? 'messages' : 'message'}</span> : lastMsg ? lastMsg.msg : 'Tap to chat'}</p>
                            </div>
